...
\n\
# Seconds hidden before clearing the main launcher query. -1 keeps the current\n\
# behavior; positive values below 5 fall back to -1.\n\
...
",
    ...
    fn ensure_default_config_file_appends_missing_keys_without_overwriting_existing() {
        ...
        assert!(contents.contains("alias_note=Notion|Obsidian|Notes|Apple Notes|Bear|Logseq"));
        assert!(contents.contains("launcher_query_clear_after_hide_seconds=-1"));
        assert_eq!(

...
- on macOS the drawing is also edited by dragging: hold a tile until it lifts, drop it on another to trade places, or on a gap to move it there. The drop is written back into `layout` in `~/.look/super-actions.toml` and nothing else in the file changes - comments and `[tiles.*]` entries stay - so a drag and a text editor are two ways of making the same edit. A drop that would cover another tile or leave the grid is refused (those tiles dim while dragging) and the tile springs back; a file with a tile the strip cannot show is left alone with a banner saying which, so a drag never erases a name the user still has to fix

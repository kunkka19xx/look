...
- the arrangement is the user's, in `~/.look/launchpad.toml`: a drawing of the grid where each line is a row and each name a cell. Repeat a name to make a tile span, `.` for a deliberate gap, delete a name to hide that tile, up to five rows and six columns. Seeded on first run with the default layout, so the file documents itself; `rm` it to reset. Resolved once in the core, so both shells place from the same coordinates and neither works out a span for itself

...
Runtime file: explicit `LOOK_CONFIG_PATH`, falling back to `$XDG_CONFIG_HOME/look/config` (or `~/.config/look/config`), then `~/.look/config`. Reload with `Cmd+Shift+R` / `Cmd+Shift+;` (macOS) or `Ctrl+Shift+R` / `Ctrl+Shift+;` (Linux/Windows).

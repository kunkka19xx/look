...
- `query_retention_seconds` (how long the main query survives while Look is hidden, in seconds; the first open past it returns to the empty home screen; `-1` keeps the query indefinitely and is the default; positive values must be at least 5)
